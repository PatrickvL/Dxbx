...
type
  ...
  private
    ...
    function TamanoDirectorio(Folder: TListaContenido): Integer;
    //function Vacio(Folder: string): Integer;
    function EscanearXISO(Imagen: TFilestream; Folder: TListaContenido; var SigSectorVacio: Int64): Boolean;
...
function TAdminXISO.EscanearXISO(Imagen: TFilestream; Folder: TListaContenido; var SigSectorVacio: Int64): Boolean;
var
  Buffer: Pointer;
  B1: PByteArray64;
  B2: PByteArray256;
...
begin
  ...
  begin
    ...
    Entrada.Directorio := AdminFicheros.EsDirectorio(Folder.Entrada[i]);
    Entrada.Attributes := Folder.Entrada[i].Attributes;
    Entrada.Contenido := Folder.Entrada[i].Contenido;
    ...
    if Entrada.Directorio then
    begin
      Entrada.Tamano := TamanoDirectorio(Folder.Entrada[i].Contenido);
  ...
  begin
    ...
    try
      Fichero := TFilestream.Create(Entrada.FullPath, fmOpenRead);
    except
      Parar := True;
      Exit;
    end;
